...
\par\noindent
There exists a third variant of the problem where only uniquely aligned reads are classified as valid. Any read which has more than one possible optimal alignment can be classified as  ambiguous and dropped. Again, there is only a small modification needed to accomplish this within the implementation. Once again we allow for branches while backtracking, by storing lists of optimal backpointers. However, this time around we cut the search whenever we discover a branching in the backtracking process. A branch represents several solutions with the same score, and the read is thus classified as unalignable.
...
\par\noindent
There is one important aspect to be considered when using the merge procedure: Every sequence which is merged in is considered a stand-alone sequence which starts at the beginning of the graph and ends at the end. This is important because it shows that even though the tool is designed for aligning short reads against a large graph it is not design to iteratively merge these in. The result would be a long full path representing a chromosome, genome or something similar and several short full paths representing the individual reads. The imagined usage of is as a part of a larger process:
\begin{enumerate}
  \item Create a graph by merging several known genetic sequences
  \item Align individual reads against the graph
  \item Create a sequence either from sequences of reads or by altering a sequence in the graph
  \item Merge in the new sequence
\end{enumerate}
The third step is not contained within the tool as it goes beyond the domain of the thesis. Even though the results from the alignments are sequences of indexes for vertices, the process of combining them should be isomorphical to a simple version of regular assembly and can thus be solved by for instance de Bruijn graphs.
\section{Parallelization}
Of the two steps, one stand out as a text book case for possible parallelization. Searching for the candidate vertices for an index is done completely separate from the other indexes. An extremly trivial parallellization of this step is implemented through splitting up the indexes into separate threads. Further parallellization is possible as the recursive searches through the trees are only dependant on the results from the vertices higher up in the trees, there is no exchange of data between the vertices of a layer. This is not implemented in the tool to keep the ``proof of concept'' presentation of the approach as conceptually simple as possible.
\end{document}