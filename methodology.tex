...
\end{corollary}
Paths spanning the entire length of a graph $G$, from $s_G$ to $t_G$, are named full paths. Every input sequence used to build the graph has a corresponding full path. \hlcyan{We also let this rule apply to all sequences which are merged in to the graph\footnote{This is discussed in more detail in section \ref{sec:merge}}.}
...
\section{\textcolor{red}{HEURISTICS}}
The second step of the algorithm does an exhaustive search over an bounded area of the solution space. We deem a string unalignable because we can no longer guarantee the optimal solution exists within that area. If we allow a lower threshold for this search than for the pruning done in the first step of the algorithm we can no longer be certain the candidate sets contain all the vertices of the optimal alignments. There is no longer an equivalence between the best alignments in the candidate graph and the original graph, which is a necessity for guaranteeing optimal results.\\
\par\noindent
The boundary which is put on the solution space can be expanded through increasing $\lambda$. This does however come at the cost of computational complexity: A larger space takes more time to search. To increase feasibility we can sacrifice the guarantee for optimality to avoid a growth in complexity. We know the candidate vertices stem from contexts which align sufficiently good against substrings of $s$, and we have found the optimal path through them. Instead of classifying the string as unalignable, the heuristical version of the algorithm simply returns the result it finds, without guaranteeing it is optimal.
\end{document}