...
\begin{table}[H]
  \centering
  \caption[The HOXD70 substitution matrix]{The HOXD70 substitution matrix \cite{scoring_pairwise}\cite{hoxd70}}
...
One major challenge when dealing with large amounts of genetic data is storage. In 1994, M. Burrows and D.J. Wheeler presented the \textit{Burrows-Wheeler Transform} as an efficient compression scheme for text strings \cite{a_block_sorting_lossless_data_compression_algorithm}. The algorithm rearranges the string by grouping together equal elements, a trait which is favorable for compression algorithms. The operation is reversible, meaning the \textcolor{red}{original string can be easily computed from the transformed one}. The approach has later been used for developing efficient, accurate alignment algorithms\cite{read_alignment_with_bwt}.
\begin{figure}[!b]
