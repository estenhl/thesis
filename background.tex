...
One major challenge when dealing with large amounts of genetic data is storage. In 1994, M. Burrows and D.J. Wheeler presented the "Burrows-Wheeler Transform" as an efficient compression scheme for text strings\cite{a_block_sorting_lossless_data_compression_algorithm}. The algorithm rearranges the string by grouping together equal elements, a trait which is favorable for compression algorithms. The operation is reversible, meaning the original string can be easily computed from the transformed one. The approach has later been used for developing efficient, accurate alignment algorithms\cite{read_alignment_with_bwt}.
\begin{figure}[!b]
...
\begin{wrapfigure}{r}{0.65\textwidth}
  \begin{mdframed}
    ...
    \begin{subfigure}[t]{\textwidth}
      ...
      \end{mdframed}
      \captionsetup{skip=-8pt}
      \subcaption{A graph which is built through alignments without allowing variation}
    \end{subfigure}
  \end{mdframed}
  \vspace*{-5mm}
  \caption{Two proposed graph models displaying flexibility (a) and rigidity (b)}
...
\begin{figure}[!b]
  ...
  \begin{subfigure}[t]{\textwidth}
	 ...
	 \label{fig:sequence_graph_example}
  	...
  	\end{subfigure}
  \caption{Various graph representations of the strings "ACGTTTACG", "ACGTGTCA" and "ACGTTACG"}
\end{figure}
\clearpage
\subsection{Mapping}
\label{sec:mapping}
