...
\textit{Sequence alignment} is the process of determining correspondence between text strings, in this case representing DNA, by mapping the elements from one to the elements of the other according to a \textit{substitution matrix} (Fig. \ref{fig:substitution matrix}) providing a \textit{mapping score}.
...
This section will very briefly present some of the technical concepts and tools mentioned throughout the thesis. The thesis requires no understanding of these elements: They are only mentioned to provide a backbone for non-technical readers interested in the implementation section and exploring the developed tool.
\subsection{Generics in java}
\subsection{Serialization}
Serialization is the process of writing data structures and objects stored in memory while executing a program to a file. The state of the program is saved through writing the values of the variables involved. The execution can later be resumed in any given state by reading and recreating the serialized objects. Java provides their own off-the-shelf serialization through the Serialization interface which contains functionality for reading and writing objects to file through filestreams. The serialized objects are transformed to sequences of bytes containing both information regarding the semantics of the type of the object and values stored in variables. More specifics regarding the serialization process can be read in the Java API documentation \cite{java_doc_serialization}.
\subsection{The JUnit library}
JUnit\cite{junit} is a Java library which provides utilities for testing components. The testing is done through asserting a set of first order logical statements, intuitively written in Java syntax, and the checking whether these assertions are correct. Examples of statements can be:
\begin{itemize}
  \item assertEquals(x, y)
  \item assertTrue(x > y)
  \item assertNotNull(object)
\end{itemize}
The library presents an opportunity to test the \textit{correctness} of a system through checking it's state. The library is however not concerned with whether this correctness is a valid measurement of the expected behaviour of the system.
\subsection{Visualizing graphs: The dot-format}
This thesis is concerned with building graphs and searching through them. Often we want to display the graphs and the results from searches to have a reference when discussing their properties. This in itself is not an easy task: Visualization of complex graphs quickly turn into an combinatorical problem of where the elements should be placed. Graphviz\cite{graphviz} is a set of tools created for solving this problem, including a stand-alone tool for Linux platforms. .dot is one of the 
\end{document}
