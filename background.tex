...
\textcolor{red}{Read errors and correction}
...
\begin{defn}[Gap penalty]
  The penalty recieved for a gap of a given length $l$. \textcolor{red}{FIX THIS}.
\end{defn}
\begin{defn}[Scoring schema]
\label{def:scoring_schema}
A structure which provides functionality for mapping bases against bases with a function $mappingScore(c_1, c_2)$ and for penalizing gaps with a function $gapPenalty(distance)$. A distance of $1$ represents no gap. The alphabet $\Sigma$ of a scoring schema is defined by the characters present in the scoring schema.
\end{defn}
\begin{defn}[Consistent scoring schema]
A scoring schema is consistent if $mappingScore(c, c) = \underset{a \in \Sigma}{max} (mappingScore(c, a))$ for all $c \in \Sigma$, and $gapPenalty(1)=0$
\end{defn}
